...
python $DEEPSDF/usage/preprocess_data.py --data_dir $DATA_DIR --source $SHAPENET/ShapeNetCore.v2/ --name ShapeNetV2 --split $DEEPSDF/examples/splits/sv2_$CLASS\_train.json --skip
...
python $DEEPSDF/usage/train_deep_sdf.py \
...
python $DEEPSDF/usage/reconstruct.py \
    -e /media/lj/TOSHIBA/dataset/DSP-SLAM/weights/deepsdf/chairs_64 \
    -c 2000 \
    --split examples/splits/sv2_$CLASS\_test.json \
    -d $TEST_DATA \
    --skip
...
python evaluate.py \
    ...
    -c 2000 \
    -d $TEST_DATA \
    -s examples/splits/sv2_$CLASS\_test.json