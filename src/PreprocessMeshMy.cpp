...
//   app.add_option("-s", sizeFileName, "Save model_size to here");  // model_size file path
  ...
  sizeFileName = npyFileName;
  std::cout << "sizeFileName" << std::endl;
  sizeFileName.erase(sizeFileName.end() - 4, sizeFileName.end());
  sizeFileName += ".txt";
  std::cout << "sizeFileName" << sizeFileName << std::endl;
  ...
  auto model_size = BoundingCubeNormalizationWithSize(geom, true);
  float max_dist = model_size[4];

  writeSizeToTXT(model_size, sizeFileName);
